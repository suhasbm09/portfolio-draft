...
const ProjectDetails = () => {
  ...
  return (
    <div className="space-y-16 py-6">
      <button
        onClick={() => navigate(-1)}
        className="inline-flex items-center gap-2 rounded-full border border-white/10 bg-white/4 px-4 py-2 text-sm text-white/75 transition hover:border-white/20 hover:bg-white/6 hover:text-white"
      >
        <FaArrowLeft /> Back
      </button>
      ...
      <section className="grid gap-8 lg:grid-cols-[1.05fr_0.95fr] lg:items-start">
        <div className="space-y-6">
          ...
          {project.featured && (
            <p className="inline-flex w-fit rounded-full border border-cyan-100/20 bg-cyan-100/10 px-4 py-2 text-[10px] font-semibold uppercase tracking-[0.35em] text-cyan-100/80">
              Flagship build
            </p>
          )}
          ...
          <div className="flex flex-wrap gap-3">
            {project.links?.github && (
              <a
                ...
                className="inline-flex items-center gap-2 rounded-full border border-white/10 bg-white/4 px-5 py-3 text-sm font-medium text-white/80 transition hover:border-white/20 hover:bg-white/6"
              >
                <FaGithub /> GitHub
              </a>
            )}
          ...
          <div className="grid gap-4 sm:grid-cols-3">
            <div className="rounded-[1.25rem] border border-white/10 bg-white/3 p-4 backdrop-blur-xl">
            ...
            <div className="rounded-[1.25rem] border border-white/10 bg-white/3 p-4 backdrop-blur-xl">
              ...
              <p className="mt-2 text-sm leading-6 text-white/75">{project.featured ? "AI learning systems" : "Backend + Solana craft"}</p>
            </div>
          </div>
        </div>

        <div className="relative overflow-hidden rounded-4xl border border-white/10 bg-white/3 p-4 backdrop-blur-2xl">
          <div className="overflow-hidden rounded-3xl border border-white/10 bg-slate-950/50">
            {project.image ? (
              <img
                src={project.image}
                alt={`${project.title} preview`}
                className={`h-112 w-full ${project.featured ? "object-contain bg-slate-950/70 p-6" : "object-cover"}`}
              />
            ) : (
              <div className="flex h-112 items-center justify-center bg-[radial-gradient(circle_at_top,rgba(103,232,249,0.12),transparent_38%),radial-gradient(circle_at_bottom_right,rgba(129,140,248,0.1),transparent_35%),linear-gradient(135deg,rgba(255,255,255,0.04),rgba(255,255,255,0.02))]">
      ...
      <section className="grid gap-6 lg:grid-cols-3">
        <div className="rounded-[1.75rem] border border-white/10 bg-white/3 p-6 backdrop-blur-2xl lg:col-span-1">
          ...
          <div className="mt-5 space-y-3">
            {project.architecture.map((item) => (
              <div key={item} className="rounded-[1.15rem] border border-white/10 bg-white/4 p-4 text-sm leading-6 text-white/70">
                {item}
              </div>
            ))}
          </div>
        ...
        <div className="rounded-[1.75rem] border border-white/10 bg-white/3 p-6 backdrop-blur-2xl lg:col-span-2">
          ...
          <div className="mt-5 grid gap-4 md:grid-cols-2">
            {project.outcome.map((item) => (
              <div key={item} className="rounded-[1.25rem] border border-white/10 bg-white/4 p-5 text-sm leading-7 text-white/72">
                {item}
              </div>
            ))}
          </div>
          <div className="mt-6 rounded-[1.25rem] border border-white/10 bg-white/3 p-5 text-sm leading-7 text-white/72">
            {project.featured
              ? "Signum unifies tutoring, retrieval, and assessment integrity into a single product experience."
              : "The key design move was treating the product as a trustworthy workflow, not just a feature demo."}
          </div>
        </div>
      </section>

      <section className="rounded-[1.75rem] border border-white/10 bg-white/3 p-6 backdrop-blur-2xl">
        ...
        <div className="mt-6 flex flex-wrap gap-3">
          {project.stack.map((item) => (
            <span key={item} className="rounded-full border border-white/10 bg-white/4 px-4 py-2 text-sm text-white/75">
              {item}
            </span>
          ))}
        </div>
        ...
        <div className="grid gap-6 lg:grid-cols-3">
          {relatedProjects.map((item) => (
            <Link
              key={item.key}
              to={`/project/${item.key}`}
                className="rounded-3xl border border-white/10 bg-white/4 p-5 transition hover:-translate-y-1 hover:border-white/15 hover:bg-white/6"
            >
