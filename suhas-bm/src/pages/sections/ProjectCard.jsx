...
function ProjectCard({ proj }) {
  ...
  return (
    <Motion.article
      ...
      className="group relative flex h-full flex-col overflow-hidden rounded-[2rem] border border-white/10 bg-white/[0.02] backdrop-blur-2xl transition duration-300 hover:border-white/15 hover:bg-white/[0.04]"
    >
      {hasImage && (
        <div className="relative h-52 overflow-hidden bg-slate-950/50">
          <img
      ...
      {!hasImage && (
        <div className="flex h-52 items-end bg-[radial-gradient(circle_at_top_left,rgba(103,232,249,0.12),transparent_40%),radial-gradient(circle_at_bottom_right,rgba(129,140,248,0.1),transparent_40%),linear-gradient(135deg,rgba(255,255,255,0.04),rgba(255,255,255,0.02))] p-6">
          <div>
      ...
      <div className="flex flex-1 flex-col gap-5 p-6">
        <div className="space-y-2">
          ...
          <h3 className="text-2xl font-semibold tracking-tight text-white">{proj.title}</h3>
        </div>

        <p className="text-sm leading-7 text-white/68">{proj.summary}</p>
        ...
        <div className="rounded-[1.25rem] border border-white/10 bg-white/[0.02] p-4">
          ...
          <p className="mt-2 text-sm font-medium leading-6 text-white/82">{proj.impact}</p>
        </div>
        ...
        <div className="flex flex-wrap gap-2">
          {proj.stack.map((t) => (
            <span
              key={t}
              className="rounded-full border border-white/10 bg-white/[0.03] px-3 py-1 text-xs font-medium text-white/76"
            >
              {t}
            </span>
          ))}
